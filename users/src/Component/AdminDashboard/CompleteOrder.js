...
const CompleteOrder = () => {
    ...
    const getUser = async () => {
      let result = await fetch("http://35.77.222.89:4003/order");
  ...
  const CompleteOrder = async () => {
    let result = await fetch("http://35.77.222.89:4003/completeOrder");
    result = await result.json();
    setComplete(result);
  };
  useEffect(() => {
  ...
  async function editData() {
    ...
    let result = await fetch(`http://35.77.222.89:4003/order/${id}`, {
      method: "put",
      headers: {
  ...
  const handleClick = async (item) => {
    const response = await fetch(
      "http://35.77.222.89:4003/MoreOrder/" + item.UserName
    );
