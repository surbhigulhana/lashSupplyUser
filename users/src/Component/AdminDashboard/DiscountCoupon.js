...
const DiscountCoupon = () => {
  ...
  useEffect(() => {
    fetch("http://54.177.55.202:4003/discount").then((result) => {
  ...
  async function deleteData(id) {
    let result = await fetch(`http://54.177.55.202:4003/discount/${id}`, {
      method: "delete",
    });
...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
  <h1 class="h3 mb-0 text-gray-800">Discount Coupon List</h1>
  <a href="/AddDiscount"><button type="button" class="btn btn-danger" style={{background:"#DD3333",border:"solid 1px #DD3333"}}>ADD DISCOUNT OFFERS</button></a>
</div>
...
<thead>
  ...
  <tr>  
  <th class="bl5">#</th>
  <th class="bl5">Coupon Name</th>
      <th class="bl5"> Coupon Type</th>
      <th class="bl5">Coupon Discount %</th>
        <th class="bl5">Coupon Discount Amount</th>
          <th class="bl5">Cart Value</th>
                    ...
                    {data && data
                      .filter((val) => {
                        if (search == "") {
                          ...
                          val.CouponName.toLowerCase().includes(search.toLowerCase())
                        ) {
                          return val;
                        }
                        ...
                        <tr key={item._id}>
                          ...
                          <td data-label="firstName">{item.CouponName}</td>
                          <td data-label="LastName" >{item.CouponType}</td>
                          <td data-label="LastName" >{item.DiscountPercent} %</td>
                          <td data-label="LastName" >₹ {item.DiscountAmt}</td>
                          <td data-label="LastName" >₹ {item.CartValue} </td>
