...
const Inquiry = () => {
  ...
  const getUser = async () => {
    let result = await fetch("http://18.183.38.142:4003/Inquiry");
  ...
  async function deleteData(id) {
    let result = await fetch(`http://18.183.38.142:4003/Inquiry/${id}`, {
      method: "delete",
    });
    let data = await result.json();
    console.log(data);
    fetch("http://18.183.38.142:4003/Inquiry").then((result) => {
...
<Table  striped bordered hover id="example" style={{width:"100%"}}>
        <thead>
                    ...
                    {data && data
                      ...
                      .map((item, index) => (
                        <tr key={item._id}>
                          ...
                          <td data-label="firstName">{item.UserName}</td>
                          <td data-label="LastName">{item.UserEmail}</td>
                          <td data-label="Phone">{item.Phone}</td>
                          <td data-label="Email">{item.Message}</td>
